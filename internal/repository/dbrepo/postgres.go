...
func (m *postgresDBRepo) GetUserByID(id int) (models.User, error) {
	ctx, cancel := context.WithTimeout(context.Background(), 3*time.Second)
	defer cancel()
	query := `select id, fistn_name, last_name,email, password, accesws_level, created_at, updated_at from users where id = $1`
	row := m.DB.QueryRowContext(ctx, query, id)
...
func (m *postgresDBRepo) UpdateReservation(r models.Reservation) error {
	...
	defer cancel()
	query := `
		update reservations set first_name = $1, last_name=$2, email=$3, phone=$4, updated_at=$5,  start_date=$6, end_date=$7
		where id = $8
		`
